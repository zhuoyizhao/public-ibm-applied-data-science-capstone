...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL': 
        ...
        fig = px.pie(data, values='class', names='Launch Site', title='Total Success Lauches By Site')
        return fig
    else:
        # return the outcomes piechart for a selected site
        filtered_df = filtered_df[filtered_df['Launch Site'] == entered_site]
        data = filtered_df.groupby(['class']).size().reset_index()
        fig = px.pie(data, values=0, names='class', title=f'Total Success Lauches for Site {entered_site}')
        return fig
...
def get_scatter_chart(entered_site, payload_mass):
    payload_mass_min = float(payload_mass[0])
    payload_mass_max = float(payload_mass[1])

    filtered_df = spacex_df
    filtered_df = filtered_df[(filtered_df['Payload Mass (kg)'] >= payload_mass_min) & 
                            (filtered_df['Payload Mass (kg)'] <= payload_mass_max)]
